...
if isempty(crop_rect)
    movie = isx.Movie.read(fullfile(Dir.isxd,[filename,'.isxd'])); 
...
%%%% Preprocess %%%%
path_input = fullfile(Dir.isxd,[filename,'.isxd']);
...
if ~isempty(DropFrame)
    ...
    input_movie_file = fullfile(Dir.isxd,[filename,'.isxd']);
